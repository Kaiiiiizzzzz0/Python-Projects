...
def detailsFeature(ID):
    with open("studentDetails.csv", "r") as D:
        lines = D.readlines()

    student_data = [line.strip().split(",") for line in lines[1:]] 

    student_found = False
    for studentData in student_data:
        if studentData[0] == ID:
            print(f"\nDetails for ID {studentData[0]}:")
            ...
            studentDetailsFile = f"C:\\Users\\Kaizen Magdaraog\\Desktop\\finalProject\\std{studentData[0]}details.txt"
            ...
            print(f"Student details have been saved to {studentDetailsFile}.")
            student_found = True
            break
